	...
	function nectar_portfolio_processing($atts, $content = null) {
		...
		extract(shortcode_atts(array(
			...
			if( $enable_sortable === 'true' && $horizontal_filters === 'true') {
				...
				// SCS Update on line 126 with:
				// Added arg to exclude the category name ID for Coming Soon. If someone delete and recreate that category than the ID would need to be updated here.
				// ,'exclude' => '116'
					...
					<div class="container <?php if($span_num === 'elastic-portfolio-item') { echo 'normal-container'; } ?>">
						...
						<ul>
							<?php if($filter_alignment !== 'center' && $filter_alignment !== 'left') { ?> <li id="sort-label"><?php echo (!empty($nectar_options['portfolio-sortable-text'])) ? $nectar_options['portfolio-sortable-text'] : esc_html__( 'Sort Portfolio','salient-portfolio'); ?>:</li> <?php } ?>
								...
								<?php wp_list_categories(array('title_li' => '','exclude' => '116', 'taxonomy' => 'project-type', 'show_option_none'   => '', 'walker' => new Walker_Portfolio_Filter())); ?>
							</ul>
							<div class="clear"></div>
						</div>
					</div>
						...
						<div class="col <?php echo esc_attr( $span_num ) . ' '. esc_attr( $masonry_item_sizing ) . esc_attr( $custom_project_class ); ?> element <?php echo esc_attr( $project_cats ); ?>"  data-project-cat="<?php echo esc_attr( $project_cats ); ?>" <?php if(!empty($project_accent_color)) { echo 'data-project-color="' . esc_attr( $project_accent_color ) .'"'; } else { echo 'data-default-color="true"';} ?> data-title-color="<?php echo esc_attr( $project_title_color ); ?>" data-subtitle-color="<?php echo esc_attr( $project_subtitle_color ); ?>">
							...
							<div class="inner-wrap animated" data-animation="<?php echo esc_attr( $load_in_animation ); ?>">
								...
								// Project style 2.
									...
									<div class="work-item style-2 carl1" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>">
								...
								<?php } //project style 6 
									...
									<div class="work-item style-2" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>">
										...
										<div class="work-info">
											...
											if($using_custom_content === 'on') {
                                                ...
                                                <?php if($lightbox_only !== 'true') { ?>
													...
													<?php 
														...
														// project style 7
														// SCS updates here on 2021-01-26 by Carl.
														// Condition added for any case study that contains the category "Coming Soon" assigned to render without a the parent case study link.
														// The case study will still exist but without any link. Can be access if the user knows the direct url.
														// In the case that SCS does't want the possibility of this orphan case study to be view than the password protection
														// option should be enabled in the case itself.

														// CODE CHANGE PART1 START

														$findwordcat   = 'comingsoon';
														$resultwordcat = strpos($project_cats, $findwordcat);
														$currentcat = preg_replace('/\s+/', '', $project_cats);
													?>

                                                    <?php if($resultwordcat === 0) { ?>
                                                        <a href="#" class="comingsoon"></a>
                                                    <?php } else { ?>
                                                        <a href="<?php echo esc_url( $the_project_link ); ?>"></a>
														
                                                    <?php } ?>   

													<?php // CODE CHANGE PART1 STOP ?> 
											...
											<?php 
											if(!empty($project_excerpt)) { 
												echo '<p>'. wp_kses_post( $project_excerpt );
													if ($resultwordcat === 0) { 
														echo '<br>(COMING SOON)';
													}
												echo '</p>'; 
								...
								//project style 8
									...
									<div class="work-item style-2 carl3" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>">
				...
				elseif($project_style === '2') {
					...
					if ( $the_query->have_posts() ) {
						while ( $the_query->have_posts() ) {
							...
							<div class="work-item style-2 carl4" data-custom-content="'.$using_custom_content.'">' . $project_img . '
